...
[KSCrash](https://github.com/kstenerud/KSCrash.git)是一个异常收集的开源框架.本文从宏观的角度阐述KSCrash(v1.15.21)的分层实现及架构设计.
...
KSCrashReportFilterSets: 创建一个Apple report格式的并包含系统信息和用户自定义信息的数据提取器


这一层的代码调用逻辑是:通过从KSCrashInstallation.h的公有方法上报方法`- (void) sendAllReportsWithCompletion:(KSCrashReportFilterCompletion) onCompletion`中获取抽象的sink对象,并将其转成KSCrashReportFilterPipeline对象,再由KSCrash对象携带转换后的sink对象调用KSCrash层的`- (void) sendAllReportsWithCompletion:(KSCrashReportFilterCompletion) onCompletion`上报方法,最终由转换后的sink对象调用pipe中的协议方法`- filterReports:onCompletion:`完成遍历调用KSCrashReportFilter对象数组中对应的协议方法,进行数据格式的转换操作
